...
class SimulationController:
    ...
    def initialize(
        ...
        for t in self.model.tables:
            from dynafx.dynamics.dsl import LookupTable
        ...
        if self.model.agents:
            ...
            self._abm_engine = ABMEngine(self.model.agents, seed=42)
        ...
        if self.model.queues or self.model.resources or self.model.events:
            ...
            for q in self.model.queues:
                q_obj = Queue(q.name, q.capacity, q.service_time, servers=q.servers)
                if q.service_time:
                    try:
                        from dynafx.dynamics.dsl import ExprParser, _compile_expr
