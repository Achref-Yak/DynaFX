...
from cognitive_engine.system.dsl import AuxDef, FlowDef, StockDef, SysdModel
...
class SignalChain:
    ...
    The model structure:
        ...
        raw signal → DELAY3 (detection) → SMOOTH (interpretation)
        → IF (threshold gate) → impact modifier
        → core stock outflow = base * (1 + impact)

    The core stock has a constant base inflow and an outflow that
    increases when the signal crosses the threshold.
    """
