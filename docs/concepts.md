...
The knowledge graph is the twin's *memory and identity*. It holds what the twin knows: contracts, supplier reliability, project risk, port status, obligations. In DynaFX it is organized into **named graphs** — one per information source — so provenance is preserved at the source level.
...
The ontology gives the twin *semantics*. It is what lets queries ask about concepts ("all at-risk projects") instead of raw identifiers, and what lets two datasets share meaning.
...
Inference matters because real twins combine facts from many sources with different vocabularies; inference reconciles them into a single, queryable picture.
...
Stocks are the twin's *continuous state*. Everything observable about the aggregate system is ultimately a stock or a function of stocks.
...
Flows are the twin's *dynamics*: the causal structure of the system lives in how flows are defined.
...
The bridge is what makes the twin *semantic*: knowledge steers dynamics, and dynamics feed knowledge back.

**Where it lives:** `dynafx/bridge.py` (`KBSimBridge`, `ClosedLoopReasoner`).

## Evidence

**Evidence** is a simulation result written back into the knowledge graph as a triple. Where the knowledge graph holds what the twin *believes* from data, evidence holds what the twin *observed* from its own dynamics — revenue, cost, fill rate, risk.
...
**Where it lives:** `KBSimBridge.evidence_from_result`, the evidence named graph in the twin example.
...
Scenarios are how a twin *foresees*: not one forecast, but a ranked map of possible futures.
...
Policies are how a twin *acts* on what it foresees.
...
A **digital twin** is a living simulation mirror of a real asset, system, or process, continuously connected to data about its real-world counterpart. In DynaFX, the twin is *semantic*: its data layer is a knowledge graph, its reasoning layer is the ontology + rules + scenario logic, and its dynamics layer is the simulation. It spans the maturity ladder from **L1 (sense)** — ingest and represent — to **L5 (decide)** — recommend and act.

## Cognitive Digital Twin

A **cognitive digital twin** extends the twin with run-time cognition: it **learns** at run time (results become evidence and update knowledge), **foresees** the future (scenario analysis and sensitivity over its own model), and **acts** accordingly (rules and optimization drive decisions). This follows the Digital Twin Consortium's framing of cognitive digital twins as digital twins that learn by themselves, foresee the future, and act accordingly.

DynaFX implements cognition with *explicit, auditable mechanisms* — symbolic knowledge, rules, and model-based forecasting — rather than opaque black boxes. This is what makes the platform suitable for research where every step must be reproducible and explainable.
